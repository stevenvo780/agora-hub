...
export const isGitCommitConfigured = (): boolean =>
  Boolean(S3_ENDPOINT && S3_KEY && S3_SECRET && FORGEJO_API && FORGEJO_TOKEN);
...
const s3 = (): S3Client => {
  if (!_s3) {
    _s3 = new S3Client({
      ...
      region: S3_REGION,
      credentials: { accessKeyId: S3_KEY as string, secretAccessKey: S3_SECRET as string },
      forcePathStyle: true,
    });
  }
  return _s3;
...
const readBlob = async (key: string): Promise<Buffer> => {
  const r = await s3().send(new GetObjectCommand({ Bucket: S3_BUCKET, Key: key.replace(/^\//, '') }));
  const chunks: Buffer[] = [];
  // @ts-expect-error Body es un stream en Node
  for await (const c of r.Body) chunks.push(Buffer.isBuffer(c) ? c : Buffer.from(c));
  return Buffer.concat(chunks);
};

const isRetriable = (status: number): boolean =>
  status === 0 || status === 408 || status === 425 || status === 429 || (status >= 500 && status < 600);

const forgejoPost = async (path: string, body: unknown, maxAttempts = 6): Promise<{ status: number; raw: string }> => {
  let lastStatus = 0; let lastRaw = '';
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (res.status >= 400 && isRetriable(res.status) && attempt < maxAttempts) {
        lastStatus = res.status; lastRaw = raw;
      ...
      if (attempt < maxAttempts) { await new Promise((r) => setTimeout(r, 1500 * 2 ** (attempt - 1))); continue; }
    }
  }
  return { status: lastStatus, raw: lastRaw };
};

export interface HubCommitFile {
  repoPath: string;
  operation: 'create' | 'update' | 'delete';
  storagePath?: string;   // para create/update: de dónde leer el blob en MinIO
  sha?: string;           // para update/delete: sha del blob existente en Forgejo
}

export interface HubCommitParams {
  repoFullName: string;
  branch?: string;
  message: string;
  authorName?: string;
...
export interface HubCommitResult {
  ok: boolean;
  sha?: string;
  status?: number;
  error?: string;
 ...
 */
export const hubGitCommit = async (params: HubCommitParams): Promise<HubCommitResult> => {
  const branch = params.branch || 'main';
  const totalFiles = params.files.length;
  if (!isGitCommitConfigured()) return { ok: false, error: 'git-commit no configurado en el Hub', totalFiles };
  if (totalFiles === 0) return { ok: false, error: 'sin archivos', totalFiles };
  ...
  if (built.length === 0) return { ok: false, error: 'ningún archivo legible', totalFiles };
  ...
  try { sha = (JSON.parse(r.raw) as { commit?: { sha?: string } }).commit?.sha; } catch { /* noop */ }
