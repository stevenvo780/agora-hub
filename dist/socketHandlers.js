...
const executeRateLimiter = (0, rateLimit_1.createSlidingWindowRateLimiter)({ windowMs: 10000, maxPerWindow: 1000 });
...
function handleWorkerConnection(io, socket) {
    ...
    if (existing) {
        ...
        const grace = state_1.workerDisconnectGrace.get(workspaceId);
        if (grace) {
            clearTimeout(grace);
            state_1.workerDisconnectGrace.delete(workspaceId);
        }
        const reattached = (0, sessions_1.reattachSessions)(io, existing.socketId, socket.id);
    ...
    socket.on('disconnect', () => {
        ...
        if (current?.socketId === socket.id) {
            ...
            const prev = state_1.workerDisconnectGrace.get(workspaceId);
            if (prev)
                clearTimeout(prev);
            const graceTimer = setTimeout(() => {
                state_1.workerDisconnectGrace.delete(workspaceId);
                const stillSame = state_1.workersByWorkspace.get(workspaceId);
            ...
            }, state_1.WORKER_DISCONNECT_GRACE_MS);
            state_1.workerDisconnectGrace.set(workspaceId, graceTimer);
            console.log(`⏳ Worker disconnected for Workspace: ${workspaceId} — ${state_1.WORKER_DISCONNECT_GRACE_MS}ms de gracia antes de terminar sesiones`);
        }
    });
